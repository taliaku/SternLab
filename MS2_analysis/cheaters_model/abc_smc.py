...
PARAMETERS = ['wt_with_del_p', 'wt_with_syn_p', 'del_with_wt_p', 'del_with_syn_p', 'syn_with_wt_p', 'syn_with_del_p', 'syn_with_syn_p', 'triple_wt', 'triple_del', 'triple_syn']
...
def simulation(parameter_list):
    if all([i[0] >= i[1][0] and i[0] < i[1][1] for i in  list(zip(parameter_list, [i[1] for i in PRIORS]))]):
        ...
        model_data = simulate(payoff_matrix=payoff_matrix, triple_payoff_matrix=triple_payoff_matrix, passages=MAX_PASSAGE)
        return model_data
    else:
        return np.inf
...
if __name__ == '__main__':
    ...
    prop={'dfunc':calculate_particle_diff, 
          ...
          'mp':True} 
    sampler = astroabc.ABC_class(10,10000,REAL_DATA,[0.3,0.07],15,PRIORS,**prop)
