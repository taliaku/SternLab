...
pairwise.t.test(df$fluorescence, df$sample, p.adj = "fdr", pool.SD = FALSE)
pairwise.t.test(df[df$sample != "no plasmid", ]$fluorescence, df[df$sample != "no plasmid", ]$sample, p.adj = "fdr")
...
ggsave('X://volume2//noam//gfp_1664//gfp_final.png', dpi = 800, width = 4.5, height = 3)


########
p.values = c(t.test(df[df$sample == 'no plasmid',]$fluorescence, df[df$sample == 'pUC57-WT',]$fluorescence)$p.value,
  t.test(df[df$sample == 'no plasmid',]$fluorescence, df[df$sample == 'pUC57-A1664G',]$fluorescence)$p.value,
  t.test(df[df$sample == 'pUC57-WT',]$fluorescence, df[df$sample == 'pUC57-A1664G',]$fluorescence)$p.value)
p.values_adjusted <- p.adjust(p.values, method='fdr')
