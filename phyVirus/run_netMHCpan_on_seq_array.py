...
def main():
    ...
    temp_dir = f"/sternadi/home/volume1/taliakustin/software/netMHCpan-4.0/tmp/netMHCpanXXXXXX"
    old_output = f"/sternadi/home/volume3/taliakustin/phyVirus_analysis/netMHC_by_seq/{base}/{base}.{a}.csv"
    old_output2 = f"/sternadi/nobackup/volume1/talia_temp/netMHC_by_seq/{base}/{base}.{a}.csv"

    if (os.path.isfile(old_output) and os.stat(old_output).st_size != 0):
        os.system(f"cp {old_output} {directory}")
        print("cped from old output")
        return
    elif (os.path.isfile(old_output2) and os.stat(old_output2).st_size != 0):
        os.system(f"cp {old_output2} {directory}")
        print("cped from old output2")
        return
    ...
    if (os.path.isfile(output) and os.stat(output).st_size != 0):
        ...
        return
    print("ran")
    os.system(f"/sternadi/home/volume1/taliakustin/software/netMHCpan-4.0/netMHCpan {file} -t 0.1 -l 9 -a {a} -xls -xlsfile {output} -tdir {temp_dir}")
