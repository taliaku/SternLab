...
def main():
    ...
    for f in tqdm.tqdm(freqs_files):
        ...
        df_snps = df[
            (df["rank"] == 0) & (df["ref_base"] != "-") & (df["ref_position"] >= 55) & (df["ref_position"] <= 29836) & (
                        df['base'] != '-')]
            ...
            if ref_base != '-':  # not insertion
                ...
                if base == ref_base:
                    ...
                    mutations = mutations.append(
                            {"ref_position": new_loc, "ref_base": ref_base, "base": base, "coverage": coverage,
                             'frequency': frequency, "sample": sample}, ignore_index=True)
                loc = new_loc

            if ref_base == '-' and new_loc - loc < 1:  # insertion
                con += base
                mutations = mutations.append(
                    {"ref_position": row["ref_position"], "ref_base": ref_base, "base": base, "coverage": coverage,
                     'frequency': frequency, "sample": sample}, ignore_index=True)
...
def deletion_group_filter(df, initial_threshold=0.75 * 0.5, final_threshold=0.5):
    deletions_df = df[(df["frequency"] >= initial_threshold) & (df["ref_base"] != "-") & (df["ref_position"] >= 55) & (
                df["ref_position"] <= 29836) & (df['base'] == '-')]
...
def insertion_group_filter(df, initial_threshold=0.5 * 0.75, final_threshold=0.5):
    insertions_df = df[(df["estimated_freq"] >= initial_threshold) & (df["ref_base"] == "-") & (df["base"] != "-") & (
                df["ref_position"] >= 55) & (df["ref_position"] <= 29836) & (df['estimated_read_count'] > 10)]
