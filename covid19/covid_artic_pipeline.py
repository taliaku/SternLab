...
def covid_artic_pipeline(input_dir, output_dir, alias="ARTIC_pipeline", queue="adistzachi"):
    ...
    make_dir(f"{output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/")
    ...
    # run pipeline
    cmds += f'python /sternadi/home/volume2/noam/SternLab/Python_pipeline/Project_Runner.py -o {output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer -i {output_dir}/ptrimmer_cleanup/ -r /sternadi/home/volume2/noam/covid/MN908947.fasta -x 1 -c 0 -v 1e-9'
    ...
    # create concensuses
    make_dir(f"{output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/freqs/")
    cmds = f"cp {output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/*/*.freqs.csv {output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/freqs/\n"
    cmds += f"python /sternadi/home/volume2/noam/SternLab/covid19/get_consensus_covid19.py -d {output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/freqs/ -o $(basename {output_dir})\n"
    # concat together with Wuhan reference
    cmds += f"cat /sternadi/home/volume2/noam/covid/MN908947.fasta {output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/freqs/$(basename {output_dir})_consensus_all.fasta > {output_dir}/consensuses_w_wuhan.fasta\n"
    ...
    job_id_p6 = script_runner(f"python /sternadi/home/volume2/noam/SternLab/scripts/compare_aligned_msa.py -i {output_dir}/consensuses_w_wuhan.aln -r 'MN908947.3' -o {output_dir}/consensuses_w_wuhan.mutations.csv", alias='mutations_from_msa', run_after_job=job_id_p3)
    # create big freqs file
    job_id_p7 = script_runner(f"python /sternadi/home/volume2/noam/SternLab/scripts/unite_all_freq_files.py -d '{output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/freqs/' -o '{output_dir}/python_pipeline_x1_c0_v1e-9_after_ptrimmer/freqs/all_freqs.csv'", alias='unite_freqs', run_after_job=job_id_p3)
